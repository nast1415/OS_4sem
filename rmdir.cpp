...
void rmdir(string dir, string target_dir) {
	...
	for (int i = 0; i < directories.size(); i++) {
		...
		if (number <= 0) {
			cerr << "rmdir: path is incorrect" << endl;
			return;
		}
	}

	int number_of_last_dir = number; //Number of block, where del_dir announced
	
	number = 0;
	number = find_directory_name(dir, number_of_last_dir, real_directory_name, size_of_real_name, alloc_table_array);
	if (number <= 0) {
		cerr << "rmdir: there is no directory with this name on this path" << endl;
		return;
	}

	int next_block_of_last_directory;
	int first_block_of_deleting_dir = number; //Number of the block, where our directory is placed
	
	//Check that we have null directory
	char buffer2[100];
	sprintf (buffer2, "%d", first_block_of_deleting_dir);
	...
	fread(number_of_info, sizeof(int), 1, block2);

	if (block2) {
		fclose(block2);
	}

	if (number_of_info[0] != 0) {
		cerr << "rmdir: deleting directory is not empty" << endl;
		return;
	}

	alloc_table_array[first_block_of_deleting_dir + 1] = -1;
