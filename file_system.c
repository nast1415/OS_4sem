...
struct fs_node* find_create_file_directory(const char* name, node_type type) {
    ...
    if(result == NULL) {
	printf("The result is null, started to initialize file system node\n");
        // Initialize new node, using init_file_system_node function
        init_file_system_node(&result, name, type);

	printf("We're going to fing name of the directory\n");
	...
	printf("We're going to fing node of the directory\n");
	...
	printf("We find node of the directory by name\n");        
...
void readdir(fs_node* dir) {
    if(dir == NULL) {
        printf("Directory can't be NULL");
    }
