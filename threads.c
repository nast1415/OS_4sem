...
void setup_userspace()
{
    const int tss_index = 7;

    #define DPL                (3 << 13)
    #define PRESENT            (1 << 15)
    #define TYPE_AVAILABLE     (9 << 8) 

    uint64_t base = (uint64_t) &tss;
    uint32_t limit = sizeof(tss) - 1;
	
    uint32_t part0 = ((BITS(15, 0) & base) << 16) | (BITS(15, 0) & limit);
    uint32_t part1 = (BITS(31, 24) & base) | (BITS(19, 16) & limit) | DPL | PRESENT | TYPE_AVAILABLE | ((BITS(23, 16) & base) >> 16);
    uint32_t part2 = base >> 32;
    uint32_t part3 = 0;

    uint32_t *gdt_ptr = (uint32_t *) get_gdt_ptr();
    gdt_ptr[tss_index * 8 / 4 + 0] = part0;
    gdt_ptr[tss_index * 8 / 4 + 1] = part1;
    gdt_ptr[tss_index * 8 / 4 + 2] = part2;
    gdt_ptr[tss_index * 8 / 4 + 3] = part3;
    load_tr(tss_index * 8);

    #undef DPL
    #undef PRESENT
    #undef TYPE_AVAILABLE
}
